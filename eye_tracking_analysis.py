...
def generate_pdf_report(worker_id, metrics, output_path):
    ...
    pdf.cell(0, 10, f"AOI Coverage: {metrics['AOI Coverage']}%", ln=True)
    pdf.cell(0, 10, f"Avg Fixation Duration: {metrics['Avg Fixation Duration']} ms", ln=True)
    pdf.cell(0, 10, f"Time to First Fixation (Instruction Label): {metrics['Time to First Fixation (Instruction Label)']} ms", ln=True)
